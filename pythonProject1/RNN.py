...
def remove_noise(tweet_tokens,stop_words=()):
    ...
    for token,tag in pos_tag(tweet_tokens):
        token=re.sub('http[s]?://(?:[a-zA-Z][0-9]|[$-_@.&+#]|[!*\(\),]|'\ 
                     '(?:%[0-9a-fA-F][0-9a-fA-F]))+','',token) #removes URL
        token=re.sub("(@[A-Za-z0-9_]+)","",token) #Removes the nicknames
        ...
        token=lmtzr.lemmatize(token,pos) #Lemmatizing the words 
	
	#removes punctuaries and stopwords
...
if __name__ == '__main__':
    ...
    #Count the length of the larger sentence in our dataset
    ...
    #Tokenizing data
    tokenizer=Tokenizer(num_words=max_ln)
